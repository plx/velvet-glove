...
fn errcheck_builtin_uses_isolated_workspace_adapter() {
    ...
    for required in [
        ...
        "combined = append_adapter_error(error, stop_child(error.signum))",
        "combined = append_adapter_error(error, stop_child())",
        "start_new_session=True",
        "process_group = child.pid",
        "os.killpg(process_group, signum)",
        "leader_exit_deadline = time.monotonic() + 0.25",
        "native errcheck left same-group descendants after child exit",
        "native child process group survived SIGKILL",
        "signal.pthread_sigmask(signal.SIG_BLOCK, handled_signals)",
...
fn gofumpt_builtin_stages_validated_output_and_commits_transactionally() {
    ...
    for required in [
        ...
        "selected source is not contained by a Go module",
        "[go_record[\"path\"], \"mod\", \"edit\", \"-json\", private_mod]",
        "fixed = [private_tool, \"-lang=\" + lang, \"-modpath=\" + modpath]",
        "run_child(fixed, environment, record[\"snapshot\"][\"content\"])",
        "write_exact(record, candidate)",
        "rollback_targets()",
        "signal.pthread_sigmask(signal.SIG_BLOCK, handled_signals)",
