...
if [ "$logical_program" = errcheck ] || [ "$logical_program" = go ]; then
  ...
  printf '%s\n' "${CC-}" >"$record/env-CC"
  printf '%s\n' "${CXX-}" >"$record/env-CXX"
  printf '%s\n' "${CGO_CFLAGS-}" >"$record/env-CGO_CFLAGS"
  printf '%s\n' "${CGO_CPPFLAGS-}" >"$record/env-CGO_CPPFLAGS"
  printf '%s\n' "${CGO_CXXFLAGS-}" >"$record/env-CGO_CXXFLAGS"
  printf '%s\n' "${CGO_LDFLAGS-}" >"$record/env-CGO_LDFLAGS"
  printf '%s\n' "${PKG_CONFIG-}" >"$record/env-PKG_CONFIG"
  printf '%s\n' "${CGO_ENABLED-}" >"$record/env-CGO_ENABLED"
  printf '%s\n' "${GO111MODULE-}" >"$record/env-GO111MODULE"
  printf '%s\n' "${GOAUTH-}" >"$record/env-GOAUTH"
  printf '%s\n' "${GOARCH-}" >"$record/env-GOARCH"
  printf '%s\n' "${GOARM64-}" >"$record/env-GOARM64"
  printf '%s\n' "${GOCACHE-}" >"$record/env-GOCACHE"
  printf '%s\n' "${GODEBUG-}" >"$record/env-GODEBUG"
  printf '%s\n' "${GOENV-}" >"$record/env-GOENV"
  printf '%s\n' "${GOEXPERIMENT-}" >"$record/env-GOEXPERIMENT"
  printf '%s\n' "${GOFLAGS-}" >"$record/env-GOFLAGS"
  printf '%s\n' "${GOINSECURE-}" >"$record/env-GOINSECURE"
  printf '%s\n' "${GOMAXPROCS-}" >"$record/env-GOMAXPROCS"
  printf '%s\n' "${GOMODCACHE-}" >"$record/env-GOMODCACHE"
  printf '%s\n' "${GONOPROXY-}" >"$record/env-GONOPROXY"
  printf '%s\n' "${GONOSUMDB-}" >"$record/env-GONOSUMDB"
  printf '%s\n' "${GOOS-}" >"$record/env-GOOS"
  printf '%s\n' "${GOPATH-}" >"$record/env-GOPATH"
  printf '%s\n' "${GOPRIVATE-}" >"$record/env-GOPRIVATE"
  printf '%s\n' "${GOPROXY-}" >"$record/env-GOPROXY"
  printf '%s\n' "${GOROOT-}" >"$record/env-GOROOT"
  printf '%s\n' "${GOSUMDB-}" >"$record/env-GOSUMDB"
  printf '%s\n' "${GOTELEMETRY-}" >"$record/env-GOTELEMETRY"
  printf '%s\n' "${GOTOOLCHAIN-}" >"$record/env-GOTOOLCHAIN"
  printf '%s\n' "${GOTMPDIR-}" >"$record/env-GOTMPDIR"
  ...
  printf '%s\n' "${DYLD_LIBRARY_PATH-}" >"$record/env-DYLD_LIBRARY_PATH"
  printf '%s\n' "${DYLD_PRINT_LIBRARIES-}" >"$record/env-DYLD_PRINT_LIBRARIES"
  printf '%s\n' "${DYLD_VELVET_GLOVE_POISON-}" >"$record/env-DYLD_VELVET_GLOVE_POISON"
  printf '%s\n' "${LD_LIBRARY_PATH-}" >"$record/env-LD_LIBRARY_PATH"
  printf '%s\n' "${LD_PRELOAD-}" >"$record/env-LD_PRELOAD"
  printf '%s\n' "${LD_VELVET_GLOVE_POISON-}" >"$record/env-LD_VELVET_GLOVE_POISON"
fi
